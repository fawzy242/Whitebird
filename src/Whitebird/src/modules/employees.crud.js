...
export class EmployeesCRUD {
    ...
    async exportToCSV(employees = null) {
        try {
            ...
            await confirmModal.success('Exported!', `${dataToExport.length} employee(s) exported successfully!`);
        } catch (error) {
    ...
    generateCSV(employees) {
        const headers = ['ID', 'First Name', 'Last Name', 'Email', 'Phone', 'Department', 'Position', 'Hire Date', 'Salary', 'Status', 'Address'];
     ...
     */
    validate(data) {
        const required = ['firstName', 'lastName', 'email', 'phone', 'department', 'position', 'hireDate', 'salary', 'status'];
