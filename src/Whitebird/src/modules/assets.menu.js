...
export class AssetsMenu {
    ...
    generateSampleAssets() {
        const assetNames = {
            'Computer': ['Dell Laptop XPS 15', 'MacBook Pro 16"', 'HP Desktop Workstation', 'Lenovo ThinkPad'],
        ...
        Object.keys(assetNames).forEach(category => {
            assetNames[category].forEach(name => {
                ...
                const asset = {
                    ...
                    assignedTo: status === 'In Use' ? employees[Math.floor(Math.random() * employees.length)] : null,
                    assignedDate: status === 'In Use' ? this.randomDate() : null,
                    issue: status === 'Maintenance' ? issues[Math.floor(Math.random() * issues.length)] : null,
    ...
    renderCurrentTab() {
        ...
        const currentData = this.currentTab === 'available' ? available : 
                           this.currentTab === 'inuse' ? inUse : maintenance;
